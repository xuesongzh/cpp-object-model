 ...
 *			指向虚函数的指针我们要有地方存放，存放在一个表格里面，这个表格我们就称为“虚函数表virtual
 *table[vtbl]”。 (指针里面存放的是函数的入口地址) 这个虚函数表一般是保存在可执行文件中的，在程序执行的时候装载到内存中。
 *
 *			注意：虚函数表是类产生的，不是对象产生的。虚函数表是跟着类走的。
 *
 *			(2)说说类对象，为什么类对象增加4个字节？？
 *			其实因为有了虚函数的存在，导致系统向类对象中添加了一个指针，这个指针正好指向虚函数表，这个指针叫虚函数表指针，vptr指针。
 ...
 *	5.如果有多个数据成员，为了提高CPU内存访问速度，某些编译器可能会将数据成员所占有的内存比例进行调整 ——内存对齐
       a)硬件平台限制，内存以字节为单位，不同硬件平台不一定支持任何内存地址的存取，一般可能以双字节、4字节等为单位存取内存，为了保证处理器正确存取数据，需要进行内存对齐。
       b)提高CPU内存访问速度，一般处理器的内存存取粒度都是N的整数倍，假如访问N大小的数据，没有进行内存对齐，有可能就需要两次访问才可以读取出数据，而进行内存对齐可以一次性把数据全部读取出来，提高效率。
 *	6.不管什么类型的指针，指针所占有的内存大小是固定的，虚函数表指针，32位环境是4个字节，64位环境是8个字节。
 * (2)总结类型对象大小的组成：
 *  非静态成员变量所占的内存总量以及这些成员变量之间内内存对齐所占用的额外内存，以及若有虚函数，会产生虚函数表指针vptr。
 *	当然，如果类之间是多重继承关系，并且每个父类都有虚函数，后续会探讨。
 */
