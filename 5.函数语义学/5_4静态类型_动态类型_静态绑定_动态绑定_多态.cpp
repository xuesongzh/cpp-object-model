...
int main() {
    ...
    pbase02->myfunc();  //Base::myfunc()，调用父类普通函数

    //普通成员函数是静态绑定，换句话说，myfunc() 是普通成员函数。这里到底调用父类的myfunc还是子类的myfunc取决于调用者的静态类型；
    //因为这里pbase02的静态类型是Base *，调用的是Base里的myfunc()；pderive的静态类型是Derive *，所以调用的是Derive的myfunc();

    //结论：函数的普通成员函数是静态绑定的-不应该在子类中重新定义一个继承来的非虚函数。

    //四：虚函数的动态绑定--函数缺省值是静态绑定的
    cout << "----------------" << endl;
    pderive->myvirfunc();  // Derive::myvirfunc(),value = 2
    pbase02->myvirfunc();  // Derive::myvirfunc(),value = 1
    //缺省value参数打印出来的居然是1
    ...
    pbase02 = &base;
    pbase02->myvirfunc();  // Base::myvirfunc(),value = 1
    /*
	 * 虚函数是动态绑定；换句话说，myvirfunc()是虚函数，这里到底执行哪个myvivfunc()取决于调用者的动态类型，
	 * 这里pbase02的动态类型分别Derive，Base，所以调用的也分别是Derive和Base的myvirfunc()，
	 ...
	 *(2)父类指针或者引用指向子类对象
	 *(3)当以父类指针或者引用 调用子类中重写了的虚函数时，我们就能看出来多态的表现了，因为调用的是子类的虚函数；
