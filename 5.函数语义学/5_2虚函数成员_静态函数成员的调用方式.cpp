...
int main() {
	 ...
	 *d)*就得到了虚函数的地址
	 总结：虚成员函数也有一个固定的内存地址，在编译的时候就已经确定。
	      通过对象直接调用虚函数，是不通过虚函数表。
		  通过指针或者引用调用虚函数是通过虚函数表。
	 */
	 ...
	 *b)无法直接存取类中普通的非静态成员变量，非静态成员变量通过this指针存取的
	 *c)静态成员函数不能在函数后使用const，也不能设置为virtual
	 *d)可以使用类对象，类对象指针，类名字，来调用
