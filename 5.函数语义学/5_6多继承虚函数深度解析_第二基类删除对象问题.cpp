...
int main() {
    //一：多继承下的虚函数
    //多重继承复杂性体现在后边这个基类上，存在this指针调整的问题--这里Base的this指针不需要调整，Base2的this指针需要调整
    Base2 *pb2 = new Derive();  //pb2指向的地址是经过this指针调整后的地址，pb2是指向Base2 这个类中的子对象，不是指向子对象开始位置。
    //编译器视角
    //Derive *temp = new Derive();
    //Base2 *pb2 = temp + sizeof(Base); // temp类型决定后面加多少，实际上是sizeof(Base) * sizeoeof(Base2)
	 ...
	 *d)执行delete pb2时，系统的动作会是？--一定要调用Derive类的析构函数才对。
	 ...
	 *		2.~Base2()调用父类2的虚析构函数
	 *		3.~Base()调用父类1的虚析构函数
	 *  d.4)Derive里就算没有虚析构函数，因为Base2里 有虚析构函数，编译器也会给Derive生成虚析构函数，为了调用~Base2()和~Base()虚析构函数；
	 *e)凡是涉及到继承的，所有类都要求写虚析构函数;
	 */
	...
	一般这玩意用在多重继承中（从第二个虚函数表开始可能就会有），用于this指针调整。
	这玩意其实是一段汇编代码，这段代码干两个事情：
