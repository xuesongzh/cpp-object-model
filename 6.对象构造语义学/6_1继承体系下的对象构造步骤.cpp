 ...
 *    大家千万不要在构造函数中使用诸如memcpy或者直接操作等手段，来修改虚函数表指针的值，否则，调用虚函数时就可能造成系统崩溃；
 * 三：构造函数中对虚函数的调用
...
class C : public B {
 public:
    C()
        : m_c(11) {   //注意成员变量m_c的赋值时机
        myvirfunc();  //构造函数中，这里没有走虚函数表，而是通过虚函数地址，直接调用这个虚函数（静态方式调用）
        printf("C this = %p\n", this);
        cout << "C::C()" << endl;
    }
    virtual ~C() {}
    virtual void myvirfunc() {
        myvirfunc2();  //调用myvirfunc2()是通过虚函数表调用的
    }
    virtual void myvirfunc2() {}
    int m_c;
};
