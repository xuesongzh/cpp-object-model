...
int main(void) {
    ...
    Derive derive02 = derive;  //调用拷贝构造函数
    long* pDe02 = (long*)(&derive02);
    long* VptrDerive02 = (long*)(*pDe02);
    //两个对象指向类的虚函数表

    Base base = derive;  //使用子类给父类赋值
...
/*
* (1)
* 使用子类对象初始化父类对象，子类中属于父类的那部分会被编译器切割（区分）出来，拷贝给父类对象。
* 所以Base base = derive;
* 做了2个事情
*	1.生成一个base对象
*	2.使用derive来初始化base对象，编译器给我们做了一个选择，derive的虚函数指针值
*	并没有覆盖base对象的虚函数表指针值，编译器帮助我们做了这一点。
* (2)oo 面向对象和ob 基于对象概念
...
*		但是OB的设计灵活性就差。
*	c++既支持面向对象程序设计（继承，多态），也支持基于对象程序设计。
*/
