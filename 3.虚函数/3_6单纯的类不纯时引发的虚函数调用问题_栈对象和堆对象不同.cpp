...
class X {
 public:
    ...
    virtual void myVirtualFunction() {
        cout << "普通虚函数myVirtualFunction被执行" << endl;
    }
};
...
int main(void) {
    ...
    //虚函数指针被清空，但是还是可以调用虚函数
    x0.myVirtualFunction();

    //在堆上分配内存创建对象，能否调用虚函数？？ --new出来的对象（堆上）无法调用虚函数
    X *px = new X();   //构造函数可以成功调用
    px->myFunction();  //ok
    //px->myVirtualFunction();//error 会引发异常,无法调用虚函数，--是通过虚函数表指针，从虚函数表中找到对应的函数执行。
    //delete px;//error 也无法调用虚析构函数
	...
	每次运行i的地址都发生改变，但是每次运行myFunction 和 myVirtualFunction的地址都不发生改变。
	...
	动态联编：是在程序运行时，根据实际情况，动态把调用语句和被调用函数绑定到一起，动态联编一般只在多态和虚函数的情况下才会发生。
		x0.myVirtualFunction();的函数调用不叫多态，虚函数指针被清空，也可以直接调用虚函数。
		px->myVirtualFunction();是通过虚函数表调用。

	总结：
	    1.在栈上分配的对象，是不通过虚函数表调用虚函数，是静态联编，所以类对象的虚函数表指针被清空也可以调用虚函数
		2.在堆上通过new分配的对象，是通过虚函数表指针，找虚函数表对应函数来调用，虚函数表指针不能清空，也就是构造函数不能使用memset和memcpy
		3.虚函数，多态，这种概念是专门给指针或者引用使用的
	 */
...
* 同时这种隐藏的成员变量的增加或者赋值的时机往往都是在执行构造函数或者拷贝构造函数的函数体开始之前。
* 那么你如果使用memset，memcpy可能把编译器给隐藏变量(虚函数表指针)的值给清空了(或者覆盖了)。
